...
float3 getLightPos()
{
    return l_rayGen.EyePt + float3(20, 20, 0);
}
    ...
    TriAttributes    attr)
{
    ...
    float3 normal = normalize(mul(input.Norm, g_cb.WorldITs[instanceIdx]));
    float3 hitPos = hitWorldPosition();
