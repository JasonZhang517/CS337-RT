...
struct CBGlobal
{
	XMFLOAT3X4	WorldITs[VRayTracer::NUM_MESH - 1];
	float		WorldIT[11];
	uint32_t	FrameIndex;
};
...
struct CBMaterial
{
	...
	XMFLOAT4	RoughMetals[VRayTracer::NUM_MESH];
};

const wchar_t* VRayTracer::HitGroupNames[] = { L"hitGroupReflection", L"hitGroupDiffuse" };
const wchar_t* VRayTracer::RaygenShaderName = L"raygenMain";
const wchar_t* VRayTracer::ClosestHitShaderNames[] = { L"closestHitReflection", L"closestHitDiffuse" };
const wchar_t* VRayTracer::MissShaderName = L"missMain";
...
void VRayTracer::UpdateFrame(uint8_t frameIndex, CXMVECTOR eyePt, CXMMATRIX viewProj, float timeStep)
{
...
void VRayTracer::Render(const RayTracing::CommandList* pCommandList, uint8_t frameIndex)
{
	renderGeometry(pCommandList, frameIndex);
	rayTrace(pCommandList, frameIndex);
}

void VRayTracer::UpdateAccelerationStructures(const RayTracing::CommandList* pCommandList, uint8_t frameIndex)
{
	// Set instance
...
void VRayTracer::renderGeometry(const RayTracing::CommandList* pCommandList, uint8_t frameIndex)
{
	// Bind the heaps
...
const Texture2D::uptr* VRayTracer::GetRayTracingOutputs() const
{
	return m_outputViews;
}

const RenderTarget::uptr* VRayTracer::GetGBuffers() const
{
	return m_gbuffers;
}

const DepthStencil::sptr VRayTracer::GetDepth() const
{
	return m_depth;
}
...
bool VRayTracer::buildAccelerationStructures(const RayTracing::CommandList* pCommandList, GeometryBuffer* pGeometries)
{
	// Set geometries
...
void VRayTracer::zPrepass(const XUSG::CommandList* pCommandList, uint8_t frameIndex)
{
...
void VRayTracer::gbufferPass(const XUSG::CommandList* pCommandList, uint8_t frameIndex, bool depthClear)
{
	// Set barriers
...
void VRayTracer::rayTrace(const RayTracing::CommandList* pCommandList, uint8_t frameIndex)
{
