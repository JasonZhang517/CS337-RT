...
struct CBMaterial
{
	XMFLOAT4	BaseColors[PRayTracer::NUM_MESH];
	XMFLOAT4	RoughMetals[PRayTracer::NUM_MESH];
};

const wchar_t* PRayTracer::HitGroupNames[] = { L"hitGroupReflection", L"hitGroupDiffuse" };
const wchar_t* PRayTracer::RaygenShaderName = L"raygenMain";
const wchar_t* PRayTracer::ClosestHitShaderNames[] = { L"closestHitReflection", L"closestHitDiffuse" };
const wchar_t* PRayTracer::MissShaderName = L"missMain";

PRayTracer::PRayTracer(const RayTracing::Device::sptr& device) :
	m_device(device),
	m_instances()
{
...
PRayTracer::~PRayTracer()
{
}

bool PRayTracer::Init(
...
void PRayTracer::UpdateFrame(uint8_t frameIndex, CXMVECTOR eyePt, CXMMATRIX viewProj, float timeStep)
{
...
void PRayTracer::Render(const RayTracing::CommandList* pCommandList, uint8_t frameIndex)
{
	renderGeometry(pCommandList, frameIndex);
	rayTrace(pCommandList, frameIndex);
}

void PRayTracer::UpdateAccelerationStructures(const RayTracing::CommandList* pCommandList, uint8_t frameIndex)
{
	// Set instance
...
void PRayTracer::renderGeometry(const RayTracing::CommandList* pCommandList, uint8_t frameIndex)
{
	// Bind the heaps
...
bool PRayTracer::createInputLayout()
{
...
bool PRayTracer::createPipelineLayouts()
{
...
bool PRayTracer::createPipelines(Format rtFormat, Format dsFormat)
{
	auto vsIndex = 0u;
	auto psIndex = 0u;
	auto csIndex = 0u;
...
bool PRayTracer::buildAccelerationStructures(const RayTracing::CommandList* pCommandList, GeometryBuffer* pGeometries)
{
	// Set geometries
...
bool PRayTracer::buildShaderTables()
{
...
void PRayTracer::zPrepass(const XUSG::CommandList* pCommandList, uint8_t frameIndex)
{
...
void PRayTracer::gbufferPass(const XUSG::CommandList* pCommandList, uint8_t frameIndex, bool depthClear)
{
	// Set barriers
...
void PRayTracer::rayTrace(const RayTracing::CommandList* pCommandList, uint8_t frameIndex)
{
